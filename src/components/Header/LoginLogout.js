...
const LoginLogout = props => {
    ...
    const { user, iconHeight, loggingOut, handleLogin, handleLogout } = props;

    // ICONS
    // Login
    const Login = (
        <svg className="iconLogin" width={iconHeight} height={iconHeight} viewBox="0 0 24 24" onClick={handleLogin} data-testid="iconLogin">
    ...
    // Logout
    const Logout = loggingOut ? <Skeleton containerTestId="logging-out" /> : (
        <svg className="iconLogout" width={iconHeight} height={iconHeight} viewBox="0 0 24 24" onClick={handleLogout} data-testid="iconLogout">
