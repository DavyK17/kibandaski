...
const getUser = async(req, res) => {
    try {
        let result = await pool.query("SELECT id, first_name, last_name, phone, email FROM users WHERE id = $1", [req.user.id]);
        res.status(200).json({
            id: result.rows[0].id,
...
const updateUser = async(req, res) => {
    try {
        ...
        let result = await pool.query("SELECT first_name, last_name, phone, email, password FROM users WHERE id = $1", [req.user.id]);
