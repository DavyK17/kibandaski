...
const getOrders = async(req, res) => {
    ...
    try {
        if (req.query.id) { // GET ORDER BY ID
            ...
            let result = await pool.query("SELECT id, status FROM orders WHERE id = $1 AND user_id = $2", [id, userId]);
            ...
            let result = await pool.query("SELECT id, status FROM orders WHERE user_id = $1 ORDER BY created_at DESC", [userId]);
