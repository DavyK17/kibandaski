...
const getOrders = async(req, res) => {
    try {
        if (req.query.id) { // Get order by ID
            let result = await client.query("SELECT id, status FROM orders WHERE id = $1 AND user_id = $2", [req.query.id, req.user.id]);
...
const cancelOrder = async(req, res) => {
    ...
    try {
        let result = await client.query("UPDATE orders SET status = 'cancelled' WHERE id = $1 AND user_id = $2 RETURNING id", [req.query.id, req.user.id]);
