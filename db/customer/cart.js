...
const getCart = async(req, res) => {
    ...
    try { // Get cart
        ...
        // Send error if cart does not exist (albeit somehow, because users can't create them)
        ...
        // Add each item in the cart to array in cart object
...
const addToCart = async(req, res) => {
    ...
    if (typeof quantity === "string") {
        // If quantity is a string, trim, validate for numeric values and convert to number
    ...
    try { // Get cart
        ...
        if (result.rows.length > 0) {
            // Add requested quantity to existing quantity in the cart
...
const emptyCart = async(req, res) => {
    ...
    try { // Get cart
        ...
        if (result.rows.length === 0) {
            return res.status(403).send("Error: Your cart is already empty.");
        }
