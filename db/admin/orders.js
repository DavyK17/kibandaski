...
const getOrderById = async(req, res) => {
    try {
        let result = await client.query("SELECT id, status FROM orders WHERE id = $1", [req.params.orderId]);
        if (result.rows.length === 0) return res.status(404).send("Error: This order does not exist.");

        let order = {...result.rows[0], items: [] };

        result = await client.query("SELECT order_items.product_id AS product_id, order_items.quantity AS quantity, (order_items.quantity * products.price) AS total_cost FROM order_items JOIN products ON order_items.product_id = products.id WHERE order_items.order_id = $1", [req.params.orderId]);
...
const fulfillOrder = async(req, res) => {
    try {
        let result = await client.query("UPDATE orders SET status = 'fulfilled' WHERE id = $1 RETURNING id", [req.params.orderId]);
